...
import run.gleam.GleamTask
import run.gleam.openapiext.DelayedBackgroundableProcessIndicator
import run.gleam.openapiext.checkIsDispatchThread
import run.gleam.openapiext.isHeadlessEnvironment
import run.gleam.openapiext.isUnitTestMode
import run.gleam.stdext.exhaustive
import java.util.function.BiConsumer

/** Inspired by [com.intellij.openapi.progress.BackgroundTaskQueue] */
class GleamBackgroundTaskQueue {
    ...
    fun run(task: Task.Backgroundable) {
        if (isUnitTestMode && task is GleamTask && task.runSyncInUnitTests) {
            runTaskInCurrentThread(task)
        } else {
            LOG.debug("Scheduling task $task")
            if (task is GleamTask) {
    ...
    @Synchronized
    fun cancelTasks(taskType: GleamTask.TaskType) {
        cancelableTasks.removeIf { data ->
            if (data.task is GleamTask && taskType.canCancelOther(data.task.taskType)) {
                data.cancel()
                true
            } else {
                false
    ...
    private class BackgroundableTaskData(
        ...
        override fun run(continuation: Runnable) {
            ...
            if (task is GleamTask && task.waitForSmartMode && DumbService.isDumb(task.project)) {
            ...
            val indicator = when {
                ...
                task is GleamTask && task.progressBarShowDelay > 0 ->
    ...
    companion object {
        private val LOG: Logger = logger<GleamBackgroundTaskQueue>()
    }
}
